...
reparametrize_prosa_mixedgamma(const double *theta, PROSA_PARAMETERS *stheta)
{
    ...
    stheta->kp = transform_log_mv_to_gamma_k(mu, sigma2);
    stheta->tp = transform_log_mv_to_gamma_t(mu, sigma2);

    stheta->ka = transform_log_mv_to_invgamma_k(theta[2], theta[3]);
    stheta->ta = transform_log_mv_to_invgamma_t(theta[2], theta[3]);

    stheta->ks = transform_log_mv_to_gamma_k(mu, sigma2) + 2;
    stheta->ts = transform_log_mv_to_gamma_t(mu, sigma2);
