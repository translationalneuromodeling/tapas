...
% $Id: vblm.m 19158 2013-03-23 22:03:39Z bkay $
% ------------------------------------------------------------------------------
function [q, stats, q_trace] = vblm(y, X, a_0, b_0, c_0, d_0)
...
function [q, q_trace] = invert_model(y, X, prior)
    ...
    for i = 1:nMaxIter
        ...
        if (i == nMaxIter), warning('vblm: reached %d iterations',nMaxIter); end
    end
end
