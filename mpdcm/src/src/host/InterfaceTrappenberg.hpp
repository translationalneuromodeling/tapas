...
#ifndef INTERFACETRAPPENBERG_H
#define INTERFACETRAPPENBERG_H
...
class InterfaceTrappenberg : public InterfaceHost
{
    public:
...
template < class T_Integrator >
std::vector< arma::Mat< double > > 
InterfaceTrappenberg< T_Integrator >::init_y(const mxArray *u, const mxArray *theta, 
    const mxArray *ptheta)
{
    const mwSize *st = mxGetDimensions(theta);
    int dim_x = (size_t ) *mxGetPr(mxGetField(mxGetCell(theta, 0), 0, 
                "dim_x"));
...
template < class T_Integrator >
int
InterfaceTrappenberg< T_Integrator>::integrate(
