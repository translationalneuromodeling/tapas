...
__device__
FmriRK4::FmriRK4() : FmriCuda(DIM_FMRI_X, PRELOC_SIZE_FMRI_RK4_X)
{

}

__device__
int FmriRK4::update_x(dbuff *ox, dbuff *y, dbuff *u, dbuff *nx)
{
    ...
    if ( maxx <= 0 )
    {
        switch ( threadIdx.y )
        {
            case INDEX_X:
                s = INDEX_X * ox->nr + j;
                k1->arr[s] = ptheta->de * fmri_dx(ox, y, u, theta, ptheta, j);
                break;
            case INDEX_F:
                s = INDEX_F * ox->nr + j;
    ...
    if ( maxx < 0 )
    {
        ...
        {
            case INDEX_X:
                k1->arr[s] = ptheta->de * fmri_dx(nx, y, u, theta, ptheta, j);
                break;
            case INDEX_F:
    ...
    if ( maxx < 0 )
    {
        ...
        {
            case INDEX_X:
                k1->arr[s] = ptheta->de * fmri_dx(nx, y, tu, theta, ptheta, j) 
                    + k1->arr[s] * 2;
                break;
            case INDEX_F:
