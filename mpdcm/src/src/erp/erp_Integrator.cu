...
{
    // nic is the number of columns per region
...
__device__
int
Integrator::set_u(int i, 
        DataArray *u, dbuff *bu)
{
    bu->nc = u->nic;
    bu->nr = u->nir;
...
__device__
int
Integrator::set_y(int i, 
        DataArray *y, dbuff *by)
{
...
__device__
int
Integrator::set_ptheta(int i, ErpPTheta *aptheta)
{
    ptheta->dt = aptheta->dt;
    ptheta->udt = aptheta->udt;
