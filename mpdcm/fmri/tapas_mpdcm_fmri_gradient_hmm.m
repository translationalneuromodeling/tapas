function [dfdx, ny] = tapas_mpdcm_fmri_gradient_hmm(p, u, theta, ptheta, ...
    dx, dt, sloppy)
...
np = reshape(np, 1, sp1 * (sp2 + 1));

ntheta = repmat(theta, sp2 + 1, 1);
ntheta = reshape(ntheta, 1, sp1 * (sp2 + 1));

ntheta = tapas_mpdcm_fmri_set_parameters(np, ntheta, ptheta);

ny = tapas_mpdcm_fmri_int(u, ntheta, ptheta, sloppy);
ny = reshape(ny, sp2 + 1, sp1);
...
if dx == dt
    ...
    for j = 1:sp1
        dfdx(j) = {zeros(size(ny{j}, 1), size(ny{j}, 2), sp2)};
        for i = 1:sp2
            dfdx{j}(:, :, i) = (ny{i + 1, j} - ny{1, j})*(dx/dt);
        end
    end
end
