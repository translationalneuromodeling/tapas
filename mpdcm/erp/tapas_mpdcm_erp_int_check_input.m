function tapas_mpdcm_erp_int_check_input(u, theta, ptheta)
...
fscalars = {'dim_x', 'dim_u', 'Au', 'gamma1', 'gamma2', 'gamma3', 'gamma4', ...
    'r1', 'r2', 'er1r2', 'tau_e2', 'tau_es2','tau_i2', 'tau_is2'};
farrays = {'A13', 'A23'};
...
for i = 1:numel(theta)
    ...
    assert(isfield(theta{i}, 'columns'), ...
        'mpdcm:erp:int:column', ...
        '{theta{%d} missing field column', i)
        

    for j = 1:numel(fscalars)
        tapas_mpdcm_check_input_matrix(getfield(theta{i}, 'columns'), ...
            [1, 1], fscalars{j}, i);
    end
