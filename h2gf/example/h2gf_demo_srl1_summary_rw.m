...
function h2gf_demo_srl1_summary_rw(NrIter,spec_eta)
...
for m = 1:12 %length(listFiles)
    ...
    h2gf_inf = load([maskResFolder, '/h2gf_rw_est_srl1_',configtype,'_eta',eta_label,'_', num2str(NrIter),'_',num2str(m),'.mat']);
    ...
    for s = 1:length(h2gf_inf.summary)
        ...
        srl1_estpar_h2gf.param.v_0 = srl1_estpar_h2gf.param.p_prc.v_0;
        srl1_estpar_h2gf.param.al = srl1_estpar_h2gf.param.p_prc.al;
        ...
        AllInv_srl1_h2gf.v_0(s,m)  = srl1_estpar_h2gf.param.v_0;
        AllInv_srl1_h2gf.al(s,m) = srl1_estpar_h2gf.param.al;
        
        AllInv_srl1_h2gf.ze(s,m) = srl1_estpar_h2gf.param.ze;
        
        save (['AllInv_srl1_h2gf_rw_eta',eta_label,'_', num2str(NrIter),'.mat'], '-struct','AllInv_srl1_h2gf');
...
%%v_0
figure('Color',[1 1 1]); hold on;
box_input = AllInv_srl1_h2gf.v_0';
prior_index = 1;
figure('Color',[1 1 1]); hold on;
col_input = [0.4 0.0 0.6; 0.4 0.2 0.6; 0.4 0.4 0.6; 0.4 0.6 0.6; 0.4 0.8 0.6; 0.4 1.0 0.6; ...
            1.0 1.0 0.6; 1.0 0.8 0.6; 1.0 0.6 0.6; 1.0 0.4 0.6; 1.0 0.2 0.6; 0.8 0.4 0.6; ...
...
boxplot(box_input,'colors',col_input, 'Plotstyle','compact'); hold on;
[i,j]=(max(median(box_input(:,1:length(box_input(1,:))))));
mean_v_0= mean2(AllInv_srl1_h2gf.v_0);
std_v_0 = std2(AllInv_srl1_h2gf.v_0);
...
saveas(gcf,['srl1_h2gf_v_0_boxplot_',configtype,'_eta',eta_label,'_', num2str(NrIter)],'fig');
print(['srl1_h2gf_v_0_boxplot_',configtype,'_eta',eta_label,'_', num2str(NrIter)],'-dtiff');


%%al
figure('Color',[1 1 1]); hold on;
box_input = AllInv_srl1_h2gf.al';
prior_index = 2;
col_input = [0.4 0.0 0.6; 0.4 0.2 0.6; 0.4 0.4 0.6; 0.4 0.6 0.6; 0.4 0.8 0.6; 0.4 1.0 0.6; ...
            1.0 1.0 0.6; 1.0 0.8 0.6; 1.0 0.6 0.6; 1.0 0.4 0.6; 1.0 0.2 0.6; 0.8 0.4 0.6; ...
            0.8 0.0 0.6];
boxplot(box_input,'colors',col_input, 'Plotstyle','compact'); hold on;
[i,j]=(max(median(box_input(:,1:length(box_input(1,:))))));
mean_al= mean2(AllInv_srl1_h2gf.al);
std_al = std2(AllInv_srl1_h2gf.al);

plot([0 length(h2gf_inf.summary)+1],[mean_al mean_al],'black'); hold on;
plot([0 length(h2gf_inf.summary)+1],[tapas_sgm(h2gf_inf.hgf.c_prc.priormus(prior_index,1),1) tapas_sgm(h2gf_inf.hgf.c_prc.priormus(prior_index,1),1)],'r');

title({['h2gf alpha']; ['(mean: ', num2str(mean_al),'; std: ', num2str(std_al),')']});
saveas(gcf,['srl1_h2gf_al_boxplot_',configtype,'_eta',eta_label,'_', num2str(NrIter)],'fig');
print(['srl1_h2gf_al_boxplot_',configtype,'_eta',eta_label,'_', num2str(NrIter)],'-dtiff');
